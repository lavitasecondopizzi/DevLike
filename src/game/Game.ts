...
export class Game {
  ...
  get nuzlockeScoreMultiplier(){const nuz=this as Game & {nuzlockeActive?:boolean;nuzlockeRules?:string[]};if(!nuz.nuzlockeActive)return 1;return Math.min(2,1+(nuz.nuzlockeRules?.length??0)*0.1);}
  get scoreMultiplier(){return Number((this.difficulty*this.nuzlockeScoreMultiplier).toFixed(2));}
  get scoreBreakdown(){return {normal:this.scoreNormal,elite:this.scoreElite,tappe:this.scoreTappe,commesse:this.scoreCommesse,dev:this.scoreDev};}
  private addScore(base:number){return Math.round(base*this.scoreMultiplier);}
  private registerStageReached(row:number){if(row>=1&&row<=6){this.stagesReached+=1;const points=this.addScore(300);this.score+=points;this.scoreTappe+=points;}}
  private registerBattleVictory(){if(this.pendingBattleScoreType==="elite"){this.eliteBattlesWon+=1;const points=this.addScore(250);this.score+=points;this.scoreElite+=points;}else if(this.pendingBattleScoreType==="normal"){this.normalBattlesWon+=1;const points=this.addScore(100);this.score+=points;this.scoreNormal+=points;}}
  private registerCompletedCommessa(){this.completedCommesse+=1;const survivors=this.team.filter(d=>d.hp>0&&d.stress<d.maxStress).length;const burnouts=this.team.filter(d=>d.hp>0&&d.stress>=d.maxStress).length;const kos=this.team.filter(d=>d.hp<=0).length;this.scoreDevSurvivors+=survivors;this.scoreDevBurnouts+=burnouts;this.scoreDevKOs+=kos;const points=this.addScore(1000+survivors*150+burnouts*100+kos*50);this.score+=points;this.scoreCommesse+=Math.round(1000*this.scoreMultiplier);this.scoreDev+=Math.round((survivors*150+burnouts*100+kos*50)*this.scoreMultiplier);}
