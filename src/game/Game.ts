...
export class Game {
  ...
  moveBattleStarter(fromIndex:number,toIndex=0){const d=this.team[fromIndex];if(!d||d.hp<=0||d.stress>=d.maxStress)return;const selectedId=this.team[this.selectedBattleStarterIndex]?.id;const current=this.team.splice(fromIndex,1)[0];if(!current)return;const target=Math.max(0,Math.min(toIndex,this.team.length));this.team.splice(target,0,current);this.selectedBattleStarterIndex=Math.max(0,this.team.findIndex(dev=>dev.id===selectedId));}
  confirmBattleStarter(){if(!this.pendingEnemy||!this.team[this.selectedBattleStarterIndex]||this.team[this.selectedBattleStarterIndex].hp<=0||this.team[this.selectedBattleStarterIndex].stress>=this.team[this.selectedBattleStarterIndex].maxStress)return;const first=this.team.splice(this.selectedBattleStarterIndex,1)[0];if(first)this.team.unshift(first);this.selectedBattleStarterIndex=0;this.startBattle(this.pendingEnemy);}
