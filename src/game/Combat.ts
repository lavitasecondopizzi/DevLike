...
export class Combat {
  ...
  constructor(team: Developer[], enemy: Enemy, deck: Card[], nuzlockeRules:string[] = []) { this.team=team.map(d=>({...d,items:d.items.map(item=>({...item}))})); this.enemy={...enemy,intent:{...enemy.intent}}; this.nuzlockeRules=[...nuzlockeRules]; this.drawPile=this.shuffle([...deck]); const available=this.team.findIndex(d=>d.hp>0&&d.stress<d.maxStress); if(available>=0)this.activeIndex=available; else {this.result="defeat";this.log.push("Nessun developer disponibile all'inizio del combattimento.");return;} this.log.push(`${this.active.name} entra in campo contro ${this.enemy.name}.`); this.startTurn(); }
  ...
  private conditionMatches(condition:string,dev:Developer):boolean{if(condition==="lowHp")return dev.hp<dev.maxHp*.5;if(condition==="highStress")return dev.stress>=dev.maxStress*.5;if(condition==="fullHp")return dev.hp>=dev.maxHp;if(condition==="highEnergy")return this.energy>=2;if(condition==="defending")return this.block>0;if(condition==="afterTool")return this.toolPlayedThisTurn&&this.lastToolDeveloperId===dev.id;if(condition==="everyTwoTurns")return this.turn%2===0;return false;}
  ...
  playCard(index:number):boolean{const card=this.hand[index];if(!card||this.result!=="ongoing")return false;if(!this.canPlayCard(card))return false;const costReduction=this.active.classId==="designer"&&!this.firstCardPlayed?1:0;const effectiveCost=Math.max(0,card.cost-costReduction);if(effectiveCost>this.energy)return false;this.energy-=effectiveCost;this.hand.splice(index,1);if(this.nuzlockeRules.includes("battleCardLock"))this.usedCards.add(card);this.discardPile.push(card);this.firstCardPlayed=true;this.toolPlayedThisTurn=true;this.lastToolDeveloperId=this.active.id;switch(card.effect.type){case "damage":{const damage=this.dealDamage(Math.round(card.effect.amount*(1+this.codeBoost/100)),"card");this.log.push(`${card.name}: ${damage} danni.`);break;}case "heal":this.active.hp=Math.min(this.active.maxHp,this.active.hp+card.effect.amount);this.log.push(`${card.name}: +${card.effect.amount} HP.`);break;case "removeStress":this.active.stress=Math.max(0,this.active.stress-card.effect.amount);this.log.push(`${card.name}: -${card.effect.amount} Stress.`);break;case "block":this.block+=card.effect.amount;this.log.push(`${card.name}: +${card.effect.amount} Difesa.`);break;case "codeBoost":this.codeBoost+=card.effect.amount;this.active.stress=Math.min(this.active.maxStress,this.active.stress+10);this.log.push(`${card.name}: +${card.effect.amount}% Codice, +10 Stress.`);break;}this.temporaryCodeBonus=this.active.classId==="fullstack"?1:0;this.checkBurnout();this.checkVictory();if(this.result==="ongoing")this.autoSkipIfNoAction();return true;}
  basicAction(action:"code"|"debug"|"defend"):boolean{if(this.result!=="ongoing")return false;if(action!=="defend"&&this.energy<=0)return false;if(action==="code"){if(this.nuzlockeRules.includes("noCode"))return false;this.energy-=1;let damage=this.active.code+this.itemCodeBonus+this.temporaryCodeBonus;if(this.active.classId==="senior"&&(!this.active.passive.includes("Query Optimizer")||!this.firstCodeUsed))damage+=2;damage=Math.round(damage*(1+this.codeBoost/100));const dealt=this.dealDamage(damage,"code");this.active.stress=Math.min(this.active.maxStress,this.active.stress+3);this.firstCodeUsed=true;this.log.push(`CODICE: ${dealt} danni, +3 Stress.`);}if(action==="debug"){if(this.nuzlockeRules.includes("noDebug")||this.energy<2)return false;this.energy-=2;let debug=this.active.debug+this.itemDebugBonus;if(this.active.hp<this.active.maxHp*.5&&this.active.classId==="junior")debug+=2;if(this.active.classId==="hacker"&&this.enemy.typeId==="client")debug+=3;const damage=Math.round(debug*1.45*(1+this.codeBoost/100));const dealt=this.dealDamage(damage,"debug");this.active.stress=Math.min(this.active.maxStress,this.active.stress+5);this.log.push(`DEBUG: ${dealt} danni, +5 Stress.`);}if(action==="defend"){if(this.nuzlockeRules.includes("noDefense"))return false;const freeDefense=this.active.classId==="architect"&&!this.architectDefenseUsed.has(this.active.id);if(!freeDefense)this.energy-=1;if(this.active.classId==="architect")this.architectDefenseUsed.add(this.active.id);const firstDevopsDefense=this.active.classId==="devops"&&!this.devopsDefenseUsed.has(this.active.id);const defenseBonus=firstDevopsDefense?5:0;if(this.active.classId==="devops")this.devopsDefenseUsed.add(this.active.id);this.block+=10+defenseBonus;if(this.active.classId==="devops"&&this.active.passive.includes("Auto-Scaling"))this.active.hp=Math.min(this.active.maxHp,this.active.hp+2);if(this.nuzlockeRules.includes("noStressRecovery"))this.log.push(`DIFESA: ${this.block} danni bloccati${defenseBonus?` (+5)`:``}${this.active.classId==="devops"&&this.active.passive.includes("Auto-Scaling")?" · +2 HP":""}, nessun recupero Stress.`);else{this.active.stress=Math.max(0,this.active.stress-3);this.log.push(`DIFESA: ${this.block} danni bloccati${defenseBonus?` (+5)`:``}${this.active.classId==="devops"&&this.active.passive.includes("Auto-Scaling")?" · +2 HP":""}, -3 Stress${freeDefense?" (GRATIS)":""}.`);}}this.checkBurnout();this.checkVictory();if(this.result==="ongoing")this.autoSkipIfNoAction();return true;}
  ...
  get canDefendForUi():boolean{
    ...
    return this.energy>=1||(this.active.classId==="architect"&&!this.architectDefenseUsed.has(this.active.id));
  }
  get codePreview():number{
    let damage=this.active.code+this.itemCodeBonus+this.temporaryCodeBonus;
    if(this.active.classId==="senior"&&(!this.active.passive.includes("Query Optimizer")||!this.firstCodeUsed))damage+=2;
  ...
  get defensePreview():number{return 10+(this.active.classId==="devops"&&!this.devopsDefenseUsed.has(this.active.id)?5:0);}
  ...
  private enemyAttack(){let raw=this.enemy.intent.damage;let stress=this.enemy.intent.stress;if(this.enemy.typeId==="bug"){this.enemyScaling+=1;stress+=this.enemyScaling;}if(this.enemy.typeId==="meeting"&&this.active.stress>=this.active.maxStress*.5)stress+=3;if(this.enemy.typeId==="client"&&this.turn%2===0)stress+=2;if(this.enemy.typeId==="deadline"){raw+=this.turn*2;stress+=this.turn*2;}raw=Math.max(0,Math.round(raw*this.incomingMultiplier(this.active)));const damage=Math.max(0,raw-this.block);this.block=Math.max(0,this.block-raw);this.active.hp=Math.max(0,this.active.hp-damage);const stressBefore=this.active.stress;this.active.stress=Math.min(this.active.maxStress,this.active.stress+stress);if(!this.nuzlockeRules.includes("noStressRecovery")&&this.active.classId==="intern"&&!this.internStressPassiveUsed.has(this.active.id)&&stressBefore<this.active.maxStress*.75&&this.active.stress>=this.active.maxStress*.75){this.active.stress=Math.max(0,this.active.stress-10);this.internStressPassiveUsed.add(this.active.id);}this.log.push(`${this.enemy.name}: ${raw} danni. Subiti ${damage}. +${stress} Stress.`);}
