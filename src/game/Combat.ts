...
export class Combat {
  ...
  private enemyAttack(){let raw=this.enemy.intent.damage;let stress=this.enemy.intent.stress;if(this.enemy.typeId==="bug"){this.enemyScaling+=1;stress+=this.enemyScaling;}if(this.enemy.typeId==="meeting"&&this.active.stress>=this.active.maxStress*.5)stress+=3;if(this.enemy.typeId==="client"&&this.turn%2===0)stress+=2;if(this.enemy.typeId==="deadline"){raw+=this.turn*2;stress+=this.turn*2;}raw=Math.max(0,Math.round(raw*this.incomingMultiplier(this.active)));const damage=Math.max(0,raw-this.block);this.block=Math.max(0,this.block-raw);this.active.hp=Math.max(0,this.active.hp-damage);const stressBefore=this.active.stress;this.active.stress=Math.min(this.active.maxStress,this.active.stress+stress);if(!this.nuzlockeRules.includes("noStressRecovery")&&this.active.classId==="intern"&&stressBefore<this.active.maxStress*.75&&this.active.stress>=this.active.maxStress*.75)this.active.stress=Math.max(0,this.active.stress-10);this.log.push(`${this.enemy.name}: ${raw} danni. Subiti ${damage}. +${stress} Stress.`);}
