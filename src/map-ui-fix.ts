...
function refreshTeamPanel() {
  ...
  const game = getGame();
  if (!strip || !game || game.screen !== "map" || strip.querySelector(".map-active-dev")) return;
  const active = game.team[0];
  if (!active) return;
  strip.innerHTML = `<button type="button" class="map-active-dev" data-team-detail="0"><div class="map-active-head"><div><strong>${esc(active.name)}</strong><span>${esc(active.role)}</span></div><small>ATTIVO</small></div><div class="map-active-stats"><span>HP <b>${active.hp}/${active.maxHp}</b></span><span>STRESS <b>${active.stress}/100</b></span></div><div class="map-active-hint">CLICCA PER DETTAGLI</div></button>`;
}
