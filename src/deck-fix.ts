...
function renderModal(){
  ...
  g.cardCollection!.forEach(card=>{const current=counts.get(card.id);if(current)current.owned++;else counts.set(card.id,{card,owned:1,used:0});});
  ...
  const deckCards=g.deck.map((card,index)=>`<article class="deck-card ${noRemoval||g.deck.length<=5?"disabled":""}"><div class="deck-card-head"><b>${esc(card.name)}</b><strong>${card.cost} ⚡</strong></div><div class="deck-card-meta"><span>TIER ${card.tier??1}</span><span>${esc(card.effect.type.toUpperCase())}</span></div><p>${esc(card.description)}</p><button type="button" data-deck-action="remove" data-deck-index="${index}" ${noRemoval||g.deck.length<=5?"disabled":""}>RIMUOVI DAL MAZZO</button></article>`).join("");
  const collectionCards=collection.map(({card,owned,used})=>{const canAdd=!deckLocked&&g.deck.length<20&&used<owned;const sourceIndex=g.cardCollection!.findIndex(item=>item.id===card.id);return `<article class="deck-card collection-card ${!canAdd?"disabled":""}"><div class="deck-card-head"><b>${esc(card.name)}</b><strong>${card.cost} ⚡</strong></div><div class="deck-card-meta"><span>TIER ${card.tier??1}</span><span>POSSEDUTE ${owned}</span><span>MAZZO ${used}</span></div><p>${esc(card.description)}</p><button type="button" data-deck-action="add" data-deck-index="${Math.max(0,sourceIndex)}" ${canAdd?"":"disabled"}>AGGIUNGI AL MAZZO</button></article>`;}).join("");
  ...
  overlay.innerHTML=`<section class="deck-modal"><header class="deck-modal-header"><div><h2>MAZZO</h2><p>${message}</p></div><button type="button" class="deck-close" data-deck-action="close">×</button></header><div class="deck-summary"><div><b>MAZZO</b><strong>${g.deck.length}/20</strong></div><div><b>CARTE POSSEDUTE</b><strong>${g.cardCollection!.length}</strong></div><div><b>SPAZIO MINIMO</b><strong>5</strong></div></div><div class="deck-columns"><section><div class="deck-section-title"><h3>MAZZO ATTIVO</h3><span>${g.deck.length} CARTE</span></div><div class="deck-grid">${deckCards||"<p class=\"deck-empty\">Il mazzo è vuoto.</p>"}</div></section><section><div class="deck-section-title"><h3>CARTE POSSEDUTE</h3><span>${collection.length} TIPI · ${g.cardCollection!.length} COPIE</span></div><div class="deck-grid">${collectionCards||"<p class=\"deck-empty\">Nessuna carta posseduta.</p>"}</div></section></div><footer class="deck-modal-footer"><span>Le carte rimosse restano tra le carte possedute e possono essere reinserite.</span><button type="button" class="primary" data-deck-action="close">TORNA AL GIOCO</button></footer></section>`;
