...
function screen(game:Game){
  ...
  if(game.screen==="map"){
    ...
    return `<section class="panel map-panel">
      ...
      ${renderMap(game)}
      <div class="team-strip">${game.team.map((dev,i)=>teamSummary(dev,i,nuzlockeEquipmentLimit(game)===1)).join("")}</div>
    </section>`;
  }
