...
function battleSetup(game:Game){
  ...
  const team=game.team.map((dev,i)=>{
    ...
    return `<div class="battle-member ${selectedDev?"selected":""} ${!usable?"disabled-dev":""}" draggable="${usable?"true":"false"}" data-battle-dev="${i}" data-battle-order="${i}"><div class="battle-member-order">${i+1}</div>
  ...
  return `<section class="panel battle-setup battle-setup-redesign">
    <div class="battle-setup-header"><div><span class="battle-eyebrow">PREPARAZIONE COMBATTIMENTO</span><h2>ORDINA IL TEAM</h2><p>Il Developer in prima posizione sarà il primo a combattere. Riordina il team trascinando le schede.</p></div><div class="battle-threat"><span>MINACCIA</span><b>${esc(enemy.name)}</b><small>${esc(enemy.type)} · ${enemy.hp}/${enemy.maxHp} HP</small></div></div>
    <div class="battle-setup-board">
      <section class="battle-roster"><div class="battle-section-title"><span>IL TUO TEAM · ORDINE DI SCHIERAMENTO</span><small>${game.team.filter(d=>d.hp>0&&d.stress<d.maxStress).length}/${game.team.length} disponibili</small></div><div class="battle-member-list">${team}</div></section>
      <section class="battle-deploy-panel"><div class="deploy-label">PRIMO TURNO</div><div class="deploy-zone ${selected?"has-selection":""}" data-battle-drop><div class="deploy-vs">01</div><b>${esc(selected?.name??"NESSUNO")}</b><span>${selected?"È il primo Developer che entrerà in combattimento.":"Nessun Developer disponibile."}</span></div><div class="deploy-hint">Il primo della lista entra per primo. Se va KO o in Burnout, entra automaticamente il successivo.</div></section>
  ...
  return `<section class="panel menu-screen">
    <div class="menu-hero">
      <div class="menu-brand">
        ...
        <div class="menu-tagline">DEADLINES DON'T DEBUG THEMSELVES.</div>
...
function openNuzlockeRules(game:Game){const nuz=game as RenderGame;if(!nuz.nuzlockeActive)return;const rules=nuz.nuzlockeRules??[];const shell=document.querySelector<HTMLElement>(".game-shell");if(!shell||document.querySelector("[data-nuzlocke-rules-overlay]"))return;const items=rules.map(rule=>{const info=getNuzlockeRuleInfo(rule);return '<article class="header-nuzlocke-rule"><b>'+esc(info.label)+'</b><p>'+esc(info.description)+'</p></article>';}).join("");shell.insertAdjacentHTML("beforeend",'<div class="feature-overlay" data-nuzlocke-rules-overlay><div class="feature-window header-nuzlocke-window"><div class="feature-titlebar"><div><span class="codex-kicker">RUN ATTIVA // NUZLOCKE</span><h2>REGOLE ATTIVE</h2><small>'+rules.length+' regole applicate a questa run.</small></div><button type="button" class="feature-close" data-nuzlocke-rules-close>CHIUDI ×</button></div><div class="header-nuzlocke-rule-list">'+items+'</div></div></div>');shell.querySelector<HTMLElement>("[data-nuzlocke-rules-close]")?.addEventListener("click",()=>shell.querySelector("[data-nuzlocke-rules-overlay]")?.remove());}
function bind(game:Game){
  root.querySelectorAll<HTMLElement>("[data-action]").forEach(el=>el.onclick=()=>{const a=el.dataset.action;if(a==="start"){localStorage.removeItem("devlike-menu-view");clearLocalSave();game.start();}if(a==="continue"){localStorage.removeItem("devlike-menu-view");window.dispatchEvent(new CustomEvent("devlike-load-save"));return;}if(a==="leaderboard-open"){void openLeaderboard(game);return;}if(a==="nuzlocke-rules"){openNuzlockeRules(game);return;}if(a==="menu"){localStorage.setItem("devlike-menu-view","1");game.screen="menu";game.message="";return;}if(a==="confirm-start")game.confirmStartingDeveloper();if(a==="confirm-battle")game.confirmBattleStarter();if(a==="code")game.combat?.basicAction("code");if(a==="debug")game.combat?.basicAction("debug");if(a==="defend")game.combat?.basicAction("defend");if(a==="toggle-deck")game.combat?.toggleDeck();if(a==="end-turn")game.combat?.endTurn();if(a==="reward")game.chooseReward(0);if(a==="confirm-recruit")game.confirmRecruitment();if(a==="confirm-item")game.confirmItemReward();if(a==="store-item")game.storeItemReward();if(a==="open-equipment")game.openEquipment();if(a==="close-equipment")game.closeEquipment();if(a==="leaderboard-submit"){const input=document.querySelector<HTMLInputElement>("#leaderboard-nickname");const nickname=input?.value.trim()??"";const state=game as Game&{leaderboardStatus?:string};if(!nickname){state.leaderboardStatus="INSERISCI UN NICKNAME.";render(game);return;}state.leaderboardStatus="INVIO...";render(game);void submitLeaderboardEntry(buildLeaderboardEntry(game,nickname)).then(()=>{state.leaderboardStatus="RECORD SALVATO.";render(game);}).catch(()=>{state.leaderboardStatus="ERRORE: CLASSIFICA ONLINE NON DISPONIBILE.";render(game);});return;}if(a==="restart")game.restart();render(game);if(game.screen==="combat"&&game.combat?.result!=="ongoing"){game.onCombatFinished();render(game);}});
