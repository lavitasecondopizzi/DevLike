...
export function render(game:Game){root.innerHTML=`<main class="game-shell"><header><div class="brand-block"><div class="logo">DEV<span>LIKE</span></div><small>SHIP IT OR BURN OUT.</small></div><div class="header-context"><span class="header-context-label">${screenLabel(game.screen)}</span><div class="header-status"><span><b>COMMESSA</b> ${game.projectNumber}</span><span><b>TAPPA</b> ${Math.min(game.currentNode,6)}/6</span><span><b>TEMPO</b> ${game.tempo}</span><span><b>DIFFICOLTÀ</b> x${game.difficulty.toFixed(1)}</span></div></div></header>${screen(game)}</main>`;bind(game);}
